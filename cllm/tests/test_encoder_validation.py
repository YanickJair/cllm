...
from src.core.encoder import CLLMEncoder, CompressionResult
...
class ValidationRunner:
    ...
    def run_validation(self, prompts: List[Dict]) -> Dict:
        ...
        avg_compression = sum(r["compression_ratio"] for r in self.results) / total_prompts
    ...
    def _print_category_breakdown(self, category_stats):
        ...
        for category in sorted(category_stats.keys()):
            ...
            print(f"{category:20s}: {stats['passed']:2d}/{stats['total']:2d} ({coverage:5.1f}%)")
    ...
    def _print_subcategory_breakdown(self):
        ...
        for subcat in sorted(subcategory_stats.keys()):
            ...
            coverage = (stats["passed"] / stats["total"]) * 100 if stats["total"] > 0 else 0
            print(f"{subcat:25s}: {stats['passed']:2d}/{stats['total']:2d} ({coverage:5.1f}%)")
    ...
    def _print_failures(self, failed_cases):
        ...
        for i, case in enumerate(failed_cases, 1):
            ...
            print(f"Issue:      Missing both REQ and TARGET tokens")

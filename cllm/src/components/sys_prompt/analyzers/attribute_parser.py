...
from .._schemas import ExtractionField, Context, OutputFormat
...
class AttributeParser:
    ...
    def parse_extraction_fields(self, text: str) -> Optional[ExtractionField]:
        ...
        has_extraction = any(
            indicator in text_lower for indicator in extraction_indicators
        )
    ...
    def parse_contexts(self, text: str) -> list[Context]:
        ...
        if any(
            ...
            for pattern in ["non-technical", "nontechnical", "non technical"]
        ):
            ...
            pattern in text_lower for pattern in ["expert", "advanced", "specialist"]
        ):
            ...
            pattern in text_lower
            for pattern in ["business", "executives", "management"]
        ):
        ...
        if any(
            pattern in text_lower
            for pattern in ["brief", "short", "concise", "quick", "summary"]
        ):
        ...
        if "STYLE" not in added_aspects:
            if any(
                pattern in text_lower
                for pattern in ["simple", "easy", "straightforward"]
            ):
    ...
    def extract_quantifier(self, text: str) -> Optional[tuple[str, int]]:
        ...
        digit_match = re.search(
            r"\b(\d+)\s*(tips?|items?|examples?|steps?|ways?|methods?)", text_lower
        )
        if digit_match:

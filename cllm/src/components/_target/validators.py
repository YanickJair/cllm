...
def build_target_token(t: Target, omit_default_domain: bool = True) -> str:
    ...
    if domain:
        ...
        if not omit_default_domain or (default_domain is None) or (domain != default_domain):
...
class TargetValidator:
    ...
    def _is_generic_token(token: str) -> bool:
        return token.upper() in {"CONCEPT", "ANSWER", "CONTENT", "DOCUMENT", "DATA", "ITEMS", "RESULT", "SYSTEM"}
    ...
    def deduplicate(self, targets: list[Target]) -> list[Target]:
        ...
        for t in normalized:
            sig_attrs = tuple(sorted((k, str(v)) for k, v in (t.attributes or {}).items()))
        ...
        if has_specific:
            unique_targets = [t for t in unique_targets if not self._is_generic_token(t.token)]

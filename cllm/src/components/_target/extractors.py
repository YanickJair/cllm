...
from src.dictionary.en.patterns import CX_TECHNICAL_CONCEPTS, TECHNICAL_CONCEPTS
...
class BaseExtractor:
    ...
    def _build_pattern_helpers(self):
        ...
        self.calculate_pattern = make_pattern(self.vocab.REQ_TOKENS.get("CALCULATE", []))
        ...
        self.summarize_pattern = make_pattern(self.vocab.REQ_TOKENS.get("SUMMARIZE", []))
        ...
        self.transform_pattern = make_pattern(self.vocab.REQ_TOKENS.get("TRANSFORM", []))
        self.explain_pattern = make_pattern(self.vocab.REQ_TOKENS.get("EXPLAIN", []))

        # Build target detection helpers
        self.code_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("CODE", []))
        self.data_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("DATA", []))
        self.document_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("DOCUMENT", []))
        self.transcript_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("TRANSCRIPT", []))
        self.ticket_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("TICKET", []))
        self.email_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("EMAIL", []))
        self.query_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("QUERY", []))
        self.call_synonyms = set(s.lower() for s in self.vocab.TARGET_TOKENS.get("CALL", []))
        ...
        self.list_indicators = set(s.lower() for s in self.vocab.REQ_TOKENS.get("LIST", []))
        if "ITEMS" in self.vocab.TARGET_TOKENS:
            self.list_indicators.update(s.lower() for s in self.vocab.TARGET_TOKENS["ITEMS"])
    ...
    def _contains_any(self, text_lower: str, word_set: set[str], window: Optional[int] = None) -> bool:
...
class PatternExtractor(BaseExtractor):
    ...
    def _detect_concept(self, text: str, doc: Doc) -> Optional[Target]:
        ...
        explain_synonyms = set(s.lower() for s in self.vocab.REQ_TOKENS.get("EXPLAIN", []))
        if any(verb in text_lower for verb in explain_synonyms):
            if not (self._contains_any(text_lower, self.code_synonyms) or
                    self._contains_any(text_lower, self.data_synonyms) or
                    self._contains_any(text_lower, self.document_synonyms)):
                attributes = self.attribute_enhancer.enhance("CONCEPT", text, doc)
                return Target(token="CONCEPT", attributes=attributes)

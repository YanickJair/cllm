...
Agent: {agent_resolution}"""
    ...
    def generate_sentiment_example(self) -> Dict:
        ...
        if issue_category == 'complaint' or 'frustrated' in transcript.lower() or 'ridiculous' in transcript.lower():
            sentiment = 'negative'
        ...
        instruction = random.choice(self.cx_instructions['sentiment_analysis']['natural'])
...
- Recommendation: {"Prioritize resolution, consider escalation" if sentiment == 'negative' else "Standard follow-up" if sentiment == 'neutral' else "Thank customer for patience"}"""
    ...
    def generate_issue_extraction_example(self) -> Dict:
        ...
        issue = [line for line in transcript.split('\n') if line.startswith('Customer:')][0].replace('Customer:',
                                                                                                     '').strip()
...
Priority: {"URGENT" if issue_category in ['complaint', 'technical'] else "HIGH" if issue_category in ['billing', 'account'] else "NORMAL"}"""
    ...
    def generate_multi_extract_example(self) -> Dict:
        ...
        sentiment = 'negative' if issue_category in ['complaint', 'technical'] else 'neutral'
        urgency = 'high' if issue_category in ['complaint', 'technical'] else 'medium' if issue_category in ['billing',
                                                                                                             'account'] else 'low'
...
3. {"Follow up within 2 hours" if urgency == 'high' else "Follow up within 24 hours"}"""
    ...
    def save_dataset(self, dataset: List[Dict], output_file: str = "cx_cllm_training_data.jsonl"):
    ...
    def show_examples(self, dataset: List[Dict], num_examples: int = 3):
        ...
        for i, example in enumerate(samples, 1):
            print(f"Example {i}: {example['instruction_type'].replace('_', ' ').title()}")
            print("-" * 80)

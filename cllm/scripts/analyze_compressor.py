...
def load_corpus():
    ...
    with open("validation_results_100.json") as f:
        data = json.load(f)
    return [item['compressed'] for item in data
...
def analyze_pattern_potential(corpus: List[str], max_length: int = 4):
    ...
    for length in range(2, max_length + 1):
        ...
        for min_freq in [2, 3, 5, 10, 20]:
            qualifying = {seq: count for seq, count in sequence_counts.items()
                          if count >= min_freq}
    ...
    print(f"Best single configuration:")
    ...
    for combo in [(2, 2), (2, 3), (3, 3), (3, 5)]:
        ...
        if (length, freq) in results:
            ...
            print(f"  Length={length}, MinFreq={freq}: {s['tokens_saved']:5d} tokens ({s['compression_pct']:.2f}%)")

    return results


def show_top_patterns(corpus: List[str], length: int = 2, min_freq: int = 2, top_n: int = 30):
    ...
    for i, p in enumerate(patterns[:top_n], 1):
        print(f"{i:<6} {p['frequency']:<6} {p['gain']:<6} {p['value']:<7} {p['pattern']}")
...
def recommend_settings(results: dict):
    ...
    if good_configs:
        ...
        for (length, freq), stats in sorted(good_configs, key=lambda x: x[1]['compression_pct'], reverse=True):
            print(f"  Length={length}, MinFreq={freq}:")
            print(f"    → {stats['tokens_saved']} tokens saved ({stats['compression_pct']:.2f}%)")
    else:
    ...
    total_potential = sum(v['tokens_saved'] for k, v in results.items()
                          if k[1] <= 3)  # Low frequency threshold

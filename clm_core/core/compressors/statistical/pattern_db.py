...
from .schemas import Pattern, PatternStats
...
class PatternDatabase:
    ...
    def add_pattern(
        ...
        pattern: Annotated[
            Pattern,
            Doc("Pattern object to add to the database or merge into an existing entry."),
        ],
    ) -> None:
    ...
    def get_pattern(
        self,
        pattern_id: Annotated[str, Doc("Unique pattern identifier to look up in the database.")],
    ) -> Pattern:
        return self.patterns[pattern_id]

    def get_top_patterns(
        self,
        n: Annotated[int, Doc("Maximum number of top-scoring patterns to return.")] = 100,
        domain: Annotated[
            str | None,
            Doc("Optional domain name to filter patterns; returns all domains if None."),
        ] = None,
    ) -> list[Pattern]:
    ...
    def get_stats(self) -> PatternStats:
        ...
        total_tokens_saved = sum(p.frequency * p.compression_gain for p in self.patterns.values())
        avg_compression_gain = sum(p.compression_gain for p in self.patterns.values()) / len(
            self.patterns
        )
